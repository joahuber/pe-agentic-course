...
def create_fix_pr(fix_result: dict, pipeline_id: str, fix_script_path: str) -> str:
    ...
    if not os.environ.get("GITHUB_TOKEN"):
        print("[platform_agent] GITHUB_TOKEN not set — skipping PR creation.")
        return ""

    branch = f"auto-fix/{pipeline_id}"
    title = fix_result.get("github_issue_title") or f"[Auto-fix] {pipeline_id}"
    body = (
        fix_result.get("github_issue_body")
        or "Automated fix generated by the platform agent."
    )
    ...
    try:
        ...
        subprocess.run(
            ["git", "push", "origin", branch], check=True, capture_output=True
        )
        ...
        print(f"[platform_agent] Pull request created → {pr_url}")
        return pr_url
    except subprocess.CalledProcessError as exc:
        print(f"[platform_agent] Warning: PR creation failed — {exc.stderr or exc}")
        return ""
